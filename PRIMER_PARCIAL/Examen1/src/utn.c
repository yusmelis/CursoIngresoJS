...
static int getStringCuit(char* cadena, int longitud);
...
/**
 * \brief Obtiene un string valido comoCuit
 * \param pResultado Puntero al espacio de memoria donde se dejara el resultado de la funcion
 * \return Retorna 0 (EXITO) si se obtiene un numero flotante y -1 (ERROR) si no
 *
 */
...
/**
 * \brief 	Lee de stdin hasta que encuentra un '\n' o hasta que haya copiado en cadena
 * 			un máximo de 'longitud - 1' caracteres y anula los giones ingresados al cuit para evaluarlo como numerico en otra funcion.
 * \param pResultado Puntero al espacio de memoria donde se copiara la cadena obtenida
 * \param longitud Define el tamaño de cadena
 * \return Retorna 0 (EXITO) si se obtiene una cadena y -1 (ERROR) si no
 *
...
static int getStringCuit(char* cadena, int longitud)
{
	...
	if(cadena != NULL && longitud > 0)
	{
		...
		if(fgets(bufferString,sizeof(bufferString),stdin) != NULL &&
				strnlen(bufferString,sizeof(bufferString))>=9&&
				strnlen(bufferString,sizeof(bufferString))<=14)
		{
			if(bufferString[strnlen(bufferString,sizeof(bufferString))-1] == '\n')
			{
				bufferString[strnlen(bufferString,sizeof(bufferString))-1] = '\0';
			...
			if(bufferString[2]=='-')
			{
				for(i=2;i<CUIT_LEN;i++)
				{
					strncpy(bufferString[i],bufferString[i+1],sizeof(bufferString));
				}
			}

			if( bufferString[strnlen(bufferString,sizeof(bufferString))-2]=='-')
			{
				for(i=strnlen(bufferString,sizeof(bufferString))-2;i<CUIT_LEN;i++)
					{
						strncpy(bufferString[i],bufferString[i+1],sizeof(bufferString));
					}
			}
...
{
   int retorno=-1;
   char buffer[CUIT_LEN];

   if(pResultado != NULL)
   {
   	if(	getStringCuit(buffer,sizeof(buffer))==0 &&
   		esNumerica(buffer,sizeof(buffer)) &&
			strnlen(buffer,sizeof(buffer))<longitud)
   	{
   		strncpy(pResultado,buffer,longitud);
